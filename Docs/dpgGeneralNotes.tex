...
\caption{Block Jacobi fixed point iterations for pure convection - dependence on $\alpha$ regularization parameter.}
\end{figure}
...
\item \textbf{This has to be related to the fixed point/Uzawa iteration of Dahmen.}  Issues in generalization: 
\begin{itemize}
\item Addition of regularizing term $\alpha$ to graph norm (can we duplicate Dahmen's proof under $\alpha$?)
\item Addition of fluxes (nonconformity).  
\end{itemize}
\end{itemize}
