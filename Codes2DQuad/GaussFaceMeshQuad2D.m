...
face1r =  gauss.z;         face2r =  ones(NGauss,1); face3r = -gauss.z;           face4r = -ones(NGauss,1);
...
for f1=1:Nfaces
  ...
  for k1=1:K
    ...
    [grx,gsx,gry,gsy,gJ] = ...
	GeometricFactorsQuad2D(x(:,k1),y(:,k1),dVMdr,dVMds);

    % compute normals at Gauss points
    if(f1==1)  gnx = -gsx;   gny = -gsy;  end;
    if(f1==2)  gnx =  grx;   gny =  gry;  end;
    if(f1==3)  gnx =  gsx;   gny =  gsy;  end;
    if(f1==4)  gnx = -grx;   gny = -gry;  end;
...
gauss.W = [gauss.w;gauss.w;gauss.w;gauss.w]*ones(1,K);
gauss.W = gauss.W.*gauss.sJ;

% build a filter( projected penalty )
gV = Vandermonde1D(NGauss-1, gauss.z);
gF = gV*(gV'*diag(gauss.w));
gauss.filter = blkdiag(gF,gF,gF,gF);
