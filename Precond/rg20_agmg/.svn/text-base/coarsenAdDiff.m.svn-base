...
symA = 0.5*(SA+SA'); % may be too slow
...
denom = - symV + 1./(1./(diagSymA(I) - S(I)) + 1./(diagSymA(J) - S(J)) );

mu = numer./denom;

%sum(mu > 0)
mu = mu.*(diagSymA(I) - S(I) + diagSymA(J) - S(J) >= 0);

mu(I ==J ) = 1;
%sum(diagSymA(I) - S(I) + diagSymA(J) - S(J) >= 0)

ids = find( mu > 0 & mu <= kTG);
